...
static int ov_get_chipid(k210sensor_t *sensor)
{
    return 0;
}
...
static int ov_set_framesize(k210sensor_t *sensor, int width, int height)
{
    return 0;
}

void cam_ov5640_init(k210sensor_t *s)
{
    s->reset = ov_reset;
    s->get_chipid = ov_get_chipid;
    s->set_pixformat = ov_set_pixformat;
    s->set_framesize = ov_set_framesize;
}
