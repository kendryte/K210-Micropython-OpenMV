...
static void ntp_sync_thread_enrty(void *param)
{
    ...
    rt_int32_t delay = RTC_NTP_FIRST_SYNC_DELAY * 1000;
    ...
    while (1)
    {
        ...
        if (ntp_sync_to_rtc(NULL) != 0);
            delay = RTC_NTP_SYNC_PERIOD * 1000;
        else
            delay = RTC_NTP_FIRST_SYNC_DELAY * 1000;
    }
}
...
int rt_rtc_ntp_sync_init(void)
{
    ...
    thread = rt_thread_create("ntp_sync", ntp_sync_thread_enrty, RT_NULL, 4096, 26, 2);
    if (thread)
    {
